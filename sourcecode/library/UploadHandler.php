...
class UploadHandler {
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . '/usrc/',
            'upload_url' => $this->get_full_url() . '/usrc/',
            'relative_url' => '',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
...
//    }
    ...
    protected function generate_response($content, $print_response = true) {
        if ($print_response) {
            error_log("enter generate_response" . $content, 0);
    ...
    public function post($print_response = true) {
        ...
        $upload = isset($_FILES[$this->options['param_name']]) ?
                $_FILES[$this->options['param_name']] : $_FILES["file"];

//        error_log("upload read pass", 0);
...
//        error_log("file name read pass", 0);
        ...
        // root path on server
        $serverPath = dirname($this->get_server_var('SCRIPT_FILENAME'));

        if ($upload && is_array($upload['tmp_name'])) {
            // param_name is an array identifier like "files[]",
            // $_FILES is a multi-dimensional array:

//            error_log("upload is array enter", 0);

            foreach ($upload['tmp_name'] as $index => $value) {
                //set dinamic upload folder
                $newName = FileUtil::createHashFileName($upload['name'][$index]);

                $upload['name'][$index] = $newName;
                $folder = FileUtil::createFolderByFileName($serverPath, $newName);
                $this->options["upload_dir"] = $serverPath . $folder["relativePath"];
                $this->options["upload_url"] = $folder["relativePath"];
...
//                error_log("foreach upload array is array read pass", 0);
            }
        } else {
            // param_name is a single object identifier like "file",
            // $_FILES is a one-dimensional array:
            //set dinamic upload folder
            $newName = FileUtil::createHashFileName($upload['name']);
            $upload['name'] = $newName;
            $folder = FileUtil::createFolderByFileName($serverPath, $newName);
            $this->options["upload_dir"] = $serverPath . $folder["relativePath"];
            $this->options["upload_url"] = $folder["relativePath"];
            $this->filePath = $this->options["upload_dir"] . $newName;
//            error_log("========Document Root::" . $folder["path"], 0);
//            error_log("========Path server Upload::" . dirname($this->get_server_var('SCRIPT_FILENAME')), 0);
//            error_log("========File path::" . $this->filePath, 0);
//            error_log($this->filePath, 0);
//            error_log("========File path======", 0);
//            error_log("upload is single enter", 0);
            $files[] = $this->handle_file_upload(
...
//            error_log("upload is single read pass", 0);
        }
