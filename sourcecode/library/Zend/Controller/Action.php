...
abstract class Zend_Controller_Action implements Zend_Controller_Action_Interface {
    ...
    /**
     * Bind parameter into view
     * If array item has format (key1, key2): method will check key1 in formdata and bind it as key2 in view if data is valid
     *      else: key1 === key2
     * Example: array(
    ...
    protected function bindParameters( $array ) {
        ...
        foreach ( $array as $key1 => $key2 ) {
            ...
            if ( key_exists($stdKey1, $formData) && $formData[$stdKey1] != '' ) {
                $this->view->{$key2} = $formData[$stdKey1];
            }
        }
    }

    /**
     * Return array containing parameters that described in $array
     * If array item has format (key1, key2): method will check key1 in formdata and bind it as key2 in result if data is valid
     *      else: key1 === key2
     * Example: array(
     *      'create_time_from' => 'createTimeFrom'
     * );
     * @param type $array
     */
    protected function parametersToArray( $array ) {
        $retVal = array( );
        $formData = $this->getRequest()->getParams();
        foreach ( $array as $key1 => $key2 ) {
            if ( is_int($key1) ) {
            ...
            if ( key_exists($stdKey1, $formData) && $formData[$stdKey1] != '' ) {
                $retVal[$stdKey1] = $formData[$stdKey1];
            }
        }
        //return
        return $retVal;
    ...
    /**
     * Get page number of requested page depends on request parameters
     * if $keys = null, default keys = array('page', 'pageId')
     * @param type $keys
     * @return type
     */
    protected function getPageNumber( $keys = null ) {
        $retVal = 1;
        $stdKeys = $keys;
        if ( $stdKeys == null ) {
            $stdKeys = array( 'page', 'pageId' );
        }
        $formData = $this->getRequest()->getParams();
        foreach ( $stdKeys as $key) {
            if ( key_exists($key, $formData) && is_numeric($formData[$key]) && intval($formData[$key]) >= 1 ) {
                $retVal = $formData[$key];
                break;
            }
        }
